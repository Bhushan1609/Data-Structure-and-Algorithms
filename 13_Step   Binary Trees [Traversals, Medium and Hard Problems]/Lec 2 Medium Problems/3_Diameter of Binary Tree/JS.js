...
function height(root,ans){
    ...
    let a=height(root.left,ans);
    let b=height(root.right,ans);
    ans.diameterOfBinaryTree=Math.max(ans.diameterOfBinaryTree,a+b);
    return 1+Math.max(a,b);
}
function diameterOfBinaryTree(root) {
    let ans = { diameterOfBinaryTree: 0 };
    height(root,ans);
    return ans.diameterOfBinaryTree;
}
