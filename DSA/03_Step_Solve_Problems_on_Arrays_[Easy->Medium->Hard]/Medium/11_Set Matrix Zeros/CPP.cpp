...
void markRow(int i,vdd<int>&arr){
    FOR(j,len(arr[0]))arr[i][j]=-1;
}

void markCol(int j,vdd<int>&arr){
    FOR(i,len(arr))arr[i][j]=-1;
}
void callBrute(vdd<int>&arr){
    FOR(i,len(arr)) FOR(j,len(arr[0])){
        if(arr[i][j]==0){
            markRow(i,arr);
            markCol(j,arr);
        }
...
void callOptimal(vdd<int>&arr){
    ...
    FOR(i,1,len(arr)) FOR(j,1,len(arr[0])){
        if(arr[i][0]==0 or arr[0][j]==0)arr[i][j]=0;
    }
    if(arr[0][0]==0) FOR(j,len(arr[0]))arr[0][j]=0;
    if(col0==0) FOR(i,len(arr)) arr[i][0]=0;
    FOR(i,len(arr)){
        print(arr[i]);
    }
    return ;
