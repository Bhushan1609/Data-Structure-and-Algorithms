...
void printSpiral(vdd<int>&arr) {
    ...
    while (top <= bottom && left <= right) {
        FOR(i,left,right+1) ans.eb(arr[top][i]);
        top++;

        FOR(i,top,bottom+1)ans.eb(arr[i][right]);
        right--;
        if (top <= bottom) {
            FOR_R(i,left,right+1)ans.eb(arr[bottom][i]);
            bottom--;
        }
        if (left <= right) {
            FOR_R(i,top,bottom+1)ans.eb(arr[i][left]);
            left++;
        }
    }
    return print(ans);
