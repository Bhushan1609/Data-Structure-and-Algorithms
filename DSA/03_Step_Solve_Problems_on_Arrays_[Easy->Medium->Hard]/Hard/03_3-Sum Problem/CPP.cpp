...
void callBrute(vd<int>&arr){
    set<vd<int>>uniques;
    FOR(i,len(arr)) FOR(j,i+1,len(arr)) FOR(k,j+1,len(arr)){
        if(arr[i]+arr[j]+arr[k]==0){
            vd<int>temp={arr[i],arr[j],arr[k]};
            sort(all(temp));
            uniques.insert(temp);
        }
    }
