...
void callBrute(vd<int>&arr,int target){
    set<vd<int>>uniques;
    FOR(i,len(arr)) FOR(j,i+1,len(arr)) FOR(k,j+1,len(arr)) FOR(l,k+1,len(arr)){
        if(arr[i]+arr[j]+arr[k]+arr[l]==target){
            vd<int>temp={arr[i],arr[j],arr[k],arr[l]};
            sort(all(temp));
            uniques.insert(temp);
        }
    }
        ...
        FOR(j,i+1,len(arr)){
            ...
            FOR(k,j+1,len(arr)){
                ...
                if(hash.find(fourth) != hash.end()){
                    vd<int>temp={arr[i],arr[j],arr[k],fourth};
                    sort(all(temp));
                    uniques.insert(temp);
                }
                hash.insert(arr[k]);
        ...
        FOR(j,i+1,len(arr)){
            ...
            while(k<l){
                ...
                if(sum<target){
                    ...
                    k+=1;
                    l-=1;
                    while(k<l && arr[k] == arr[k-1])k+=1;
                    while(k<l && arr[l] == arr[l+1])l-=1;
                }
            }
        }
    }
