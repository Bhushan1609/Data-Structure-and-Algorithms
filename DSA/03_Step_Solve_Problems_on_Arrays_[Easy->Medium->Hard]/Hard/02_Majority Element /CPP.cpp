...
void callBetter(vd<int>&arr){
    vd<int>ans;
    int mini=int(len(arr)/3)+1;
    unordered_map<int,int>map;
    FOR(i,len(arr)){
        map[arr[i]]++;
        if(map[arr[i]]==mini)ans.eb(arr[i]);
    }
    return print(ans);
}
...
void callOptimal(vd<int>&arr){
    ...
    int mini=int(len(arr)/3)+1;
    if(cnt1>=mini)ans.eb(ele1);
    if(cnt2>=mini)ans.eb(ele2);
    return print(ans);
}

void solve(){
    vd<int>arr={1,1,1,3,3,2,2,2};
    callBrute(arr);
    callBetter(arr);
    callOptimal(arr);
}
