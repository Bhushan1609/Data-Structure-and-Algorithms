...
void callBrute(vd<int>&arr,int k){
    int cnt=0;
    FOR(i,len(arr)) FOR(j,len(arr)) {
        int xorr=0;
        FOR(k,i,j+1) xorr ^= arr[k];
        cnt+=(xorr==k);
    }
    return print(cnt);
}

void callBetter(vd<int>&arr,int k){
    int cnt=0;
    FOR(i,len(arr)) {
        int xorr=0;
        FOR(j,i,len(arr)){
...
void callOptimal(vd<int>&arr,int k){
    int cnt=0;
    ...
    FOR(i,len(arr)){
        xorr ^= arr[i];
        int needXorr=xorr ^ k;
        if(mapp.find(needXorr) != mapp.end()){
            cnt += mapp[needXorr];
        }
        mapp[xorr]+=1;
    }
