...
void callBrute(vd<int>&arr,int target){
    int n(len(arr)),ans(0);
    FOR(i,n) FOR(j,n){
        int sum=0;
        FOR(k,i,j+1){
        ...
        if(sum==target)ans=max(ans,int(j-i+1));
    }
    return print(ans);
}

void optimizedBrute(vd<int>&arr,int target){
    int n(len(arr)),ans(0);
    FOR(i,n){
        int sum=0;
        FOR(j,i,n){
...
void callBetter(vd<int>&arr,int target){
    int n(len(arr)),ans(0),sum(0);
    unordered_map<int,int>prefixSum;
    FOR(i,n){
        sum+=arr[i];
        if(arr[i]==target)ans=max(ans,1);
        if(sum==target)ans=max(ans,(int)i+1);
        int needSum=sum-target;
        if(prefixSum[needSum]){
            ans=max(ans,int(i-(prefixSum[needSum])));
        }
        if(prefixSum[sum]==0)prefixSum[sum]=i;
    }
    return print(ans);
}

void callOptimal(vd<int>&arr,int target){
    int left(0),right(0),ans(0),sum(arr[0]);
    while(right<len(arr)){
...
void solve(){
    vd<int>arr={1,2,3,1,1,1,1,4,2,3};
    int target=3;
