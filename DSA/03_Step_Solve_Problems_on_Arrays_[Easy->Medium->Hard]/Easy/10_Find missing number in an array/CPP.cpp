...
void callBrute(vd<int>&arr,int n){
    FOR(i,1,n+1){
        bool flag=0;
        FOR(j,len(arr)){
            if(arr[j]==i){
...
void callBetter(vd<int>&arr,int n){
    vd<int>hash(n+1,0);
    FOR(i,len(arr)) hash[arr[i]]++;
    FOR(i,1,len(hash)){
        if(!hash[i]) return print(i);
    }
}

void callOptimal1(vd<int>&arr,int n){
    int sum(n*(n+1)>>1);
    FOR(i,len(arr)) sum-=arr[i];
    return print(sum);
}

void callOptimal2(vd<int>&arr,int n){
    int xorr(0);    
    FOR(i,1,n+1) xorr^=i;
    FOR(i,len(arr)) xorr^=arr[i];
    return print(xorr);
}
