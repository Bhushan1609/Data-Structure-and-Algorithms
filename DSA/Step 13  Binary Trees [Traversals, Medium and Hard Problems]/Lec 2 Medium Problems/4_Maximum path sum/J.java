...
public class J{
    ...
    static int getSum(Node root,IntWrapper ans){
        if(root==null)return 0;
        int ans1=Math.max(0,getSum(root.left,ans));
        int ans2=Math.max(0,getSum(root.right,ans));
        ans.value=Math.max(ans.value,ans1+ans2+root.data);
        return Math.max(ans1,ans2)+root.data;
    }
    static int maxPathSum(Node root) {
        if(root==null)return 0;
        IntWrapper ans=new IntWrapper(Integer.MIN_VALUE);
        getSum(root,ans);
        return ans.value;
    }
