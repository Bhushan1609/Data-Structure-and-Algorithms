...
Node* build(vector<int>& inorder,int is,int ie, vector<int>& postorder,int ps,int pe,map<int,int>&mapp){
    if(is>ie or ps>pe)return NULL;
    Node* root=new Node(postorder[pe]);
    int inorderIndex=mapp[postorder[pe]];
    int numsLeft=inorderIndex-is;
    root->left=build(inorder,is,inorderIndex-1,postorder,ps,ps+numsLeft-1,mapp);
    root->right=build(inorder,inorderIndex+1,ie,postorder,ps+numsLeft,pe-1,mapp);
    return root;
}
...
void itachi_1609(){
    Node* root=buildTree({9,3,15,20,7},{9,15,7,20,3});
    levelOrder(root);
    printn();
    return;
}
