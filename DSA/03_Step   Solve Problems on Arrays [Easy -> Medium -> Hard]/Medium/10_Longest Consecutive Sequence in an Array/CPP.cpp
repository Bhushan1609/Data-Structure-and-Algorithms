...
void callBetter(vd<int>&arr){
    int lastSmaller=arr[0],cnt=1,longest=0;
    sort(all(arr));
    FOR(i,1,len(arr)){
...
void callOptimal(vd<int>&arr){
    ...
    FOR(i,len(arr))st.insert(arr[i]);
    for(auto &i:st){
        if(st.find(i-1) == st.end()){
            int cnt=1;
            int search=i;
            while(st.find(search+1) != st.end()){
                cnt+=1;
                search+=1;
            }
            longest=max(longest,cnt);
...
void solve(){
    vd<int>arr={102,4,100,1,101,3,2,1,1};
    callBrute(arr);
    callBetter(arr);
    callOptimal(arr);
}
