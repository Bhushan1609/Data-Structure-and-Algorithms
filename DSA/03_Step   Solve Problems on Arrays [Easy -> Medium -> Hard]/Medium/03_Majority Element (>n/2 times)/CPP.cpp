...
void callBrute(vd<int>&arr){
    FOR(i,len(arr)){
        ...
        if(cnt > len(arr)>>1) return print(arr[i]);
    }
    return print(-1);
}
...
void callBetter(vd<int>&arr){
    ...
    FOR(i,len(arr)) mapp[arr[i]]+=1;
    for(auto &[i,j]:mapp){
        if(j>len(arr)>>1) return print(i);
    }
    return print(-1);
}
...
void callOptimal(vd<int>&arr){
    ...
    FOR(i,len(arr))count += (arr[i] == ele );
    if(count > len(arr) >> 1) return print(ele);
    return print(-1);
}

void solve(){
    vd<int>arr={2,2,3,3,1,2,2};
    callBrute(arr);
    callBetter(arr);   
