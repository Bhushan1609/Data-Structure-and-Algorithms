...
void callBetter(vd<int>&arr){
    int missing=-1,repeating=-1;
    int n=len(arr);
    vd<int>hash(n+1,0);
    FOR(i,len(arr))hash[arr[i]]+=1;
    FOR(i,1,len(hash)){
...
void callOptimal(vd<int>&arr){
    ...
    int equation1=sumOfNarray-sumOfN;
    int equation2=sumOfSquaresarray-sumOfSquares;

    equation2/=equation1;

    repeating=(equation1+equation2)>>1;

    missing=(repeating-equation1);

    return print(repeating,missing);
}

void solve(){
    vd<int>arr={4,3,6,2,1,1};
    callBrute(arr);
    callBetter(arr);
    callOptimal(arr);
}
