...
void dfs(ll i,ll j,vdd<bool>&visited,vdd<char>& b){
    visited[i][j]=true;
    FOR(4){
        ll newx=i+fourDirection[_].fi;
        ll newy=j+fourDirection[_].se;
        if(newx>=0 and newx<b.size() and newy>=0 and newy<b[0].size() and b[newx][newy]=='O' and !visited[newx][newy]){
...
void solve(vector<vector<char>>& b) {
    ...
    FOR(m){
        if(b[0][_]=='O' and !visited[0][_])dfs(0,_,visited,b);
        if(b[n-1][_]=='O' and !visited[n-1][_]) dfs(n-1,_,visited,b);
    }
    FOR(i,0,n){
        if(b[i][0]=='O' and !visited[i][0]) dfs(i,0,visited,b);
        if(b[i][m-1]=='O' and !visited[i][m-1]) dfs(i,m-1,visited,b);
    }
    FOR(n){
        FOR(j,0,m){
            if(b[_][j]=='O' and !visited[_][j])b[_][j]='X';
        }
    }
    for(auto i:b){
        for(auto j:i)cout<<j<<" ";
