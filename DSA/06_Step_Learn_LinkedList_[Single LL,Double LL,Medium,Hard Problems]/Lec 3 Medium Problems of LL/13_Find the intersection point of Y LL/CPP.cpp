...
Node* callBrute(Node* head1,Node* head2){
...
Node* collisionPoint(Node* big,Node* small,int d){
    
    while(d--)big=big->next;
...
Node* callBetter(Node* head1,Node* head2){
    int len1=0,len2=0;
    ...
    if(len1>len2)return collisionPoint(head1,head2,len1-len2);
    return collisionPoint(head2,head1,len2-len1);
}

Node* callOptimal(Node* head1,Node* head2){
...
void solve(){
    ...
    // printLL(callBrute(head));

    // printLL(callBetter(head));

    // printLL(callOptimal(head));
}
