...
vd<pi<int,int>> callBrute(Node* head,int target){
    vd<pi<int,int>>answer;
    Node* temp1=head;
    while(temp1){
        Node* temp2=temp1->next;
        while(temp2){
            if(temp1->data+temp2->data==target) answer.pb({temp1->data,temp2->data});
            else if(temp1->data+temp2->data>target)break;
            temp2=temp2->next;
        }
        temp1=temp1->next;
    }
...
vd<pi<int,int>> callOptimal(Node* head,int target){
    vd<pi<int,int>>answer;
    Node* left=head,*right=giveTail(head);
    while(left->data < right->data){
        if((left->data+right->data)==target){
            answer.pb({left->data,right->data});
            left=left->next;
            right=right->back;
...
void solve(){
    vector<int>vec={1,2,4,5,6,8,9};
    Node* head=arrayToDLL(vec);
    printDLL(head);
    vd<pi<int,int>> answer=callBrute(head,7);
    FOR(i,len(answer)){
        cout<<answer[i].fi<<" --> "<<answer[i].se<<endl;
    }
    print("");
    answer=callOptimal(head,7);
    FOR(i,len(answer)){
        cout<<answer[i].fi<<" --> "<<answer[i].se<<endl;
    }
    return print("");
