...
void singleTraversal(Node* root){
    if(!root)return;
    stack<pi<Node*,int>>st;
    st.push({root,1});
    vd<int>pre,in,post;
    while(!st.empty() ){
       pi<Node*,int>temp=st.top();
       st.pop();
       if(temp.se==1){
        pre.eb(temp.fi->data);
        st.push({temp.fi,temp.se+1});
        if(temp.fi->left)st.push({temp.fi->left,1});
       }

       if(temp.se==2){
        in.eb(temp.fi->data);
        st.push({temp.fi,temp.se+1});
        if(temp.fi->right)st.push({temp.fi->right,1});
       }

       if(temp.se==3){
        post.eb(temp.fi->data);
       }
    }
    print(pre);
    print(in);
