...
void leftBoundary(Node* root,vd<int>&answer){
    while(root){
...
void leafBoundary(Node* root,vd<int>&answer){
    if(!root)return ;
    if(!root->left && !root->right){
        answer.eb(root->data);
    }
    leafBoundary(root->left,answer);
    leafBoundary(root->right,answer);
}

void rightBoundary(Node* root,vd<int>&answer){
    while(root){
        if(!root->left && !root->right) return ;
        answer.eb(root->data);
...
vector<int> boundaryTraversal(Node *root) {
    ...
    leafBoundary(root,answer);
    int sz=len(answer);
    rightBoundary(root->right,answer);
    reverse(answer.begin()+sz,answer.end());
    return answer;
}
