...
int height(TreeNode<int>* root,bool &isSafe){
    if(!root)return 0;
    int height_left=height(root->left,isSafe);
    int height_right=height(root->right,isSafe);
    isSafe&=(abs(height_left-height_right)<=1);
...
int main(){
    ...
    bool isSafe=true;
    height(root,isSafe);
    cout<<"Height Balanced or not : "<<isSafe<<endl;
    return 0;
}