...
void givePaths(TreeNode<int>* root,vector<int>dummy,vector<vector<int>>&Paths){
    if(!root)return ;
    if(!root->left && !root->right){
        dummy.emplace_back(root->data);
        Paths.emplace_back(dummy);
        dummy.pop_back();
        return ;
    }
    dummy.emplace_back(root->data);
    givePaths(root->left,dummy,Paths);
    givePaths(root->right,dummy,Paths);
    dummy.pop_back();
}
