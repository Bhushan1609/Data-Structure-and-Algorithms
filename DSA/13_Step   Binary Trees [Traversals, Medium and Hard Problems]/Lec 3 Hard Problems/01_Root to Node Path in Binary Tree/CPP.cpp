...
void r(Node* root,vd<int>temp,vdd<int>&answer){
    if(!root)return;
    temp.eb(root->data);
    if(!root->left && !root->right)answer.eb(temp);
    r(root->left,temp,answer);
    r(root->right,temp,answer);
    temp.pop_back();
}
...
void solve(){
    ...
    // levelOrder(root);
    // vdd<int>answer=Paths(root);
    // FOR(i,len(answer)) print(answer[i]);
