...
void makeParentMapping(TreeNode* root, unordered_map<TreeNode*,TreeNode*>&parentMapping){
    if(!root)return ;
    if(root->left)parentMapping[root->left]=root;
    if(root->right)parentMapping[root->right]=root;
    makeParentMapping(root->left,parentMapping);
    makeParentMapping(root->right,parentMapping);
}
void distanceK(TreeNode* root, TreeNode* target, int k) {
    ...
    makeParentMapping(root,parentMapping);
    q<TreeNode*>q;
    q.push(target);
    while(!q.empty()){
        int n=q.size();
        ...
        FOR(n){
            ...
            q.pop();
            visited[temp]=true;
            TreeNode* parent=parentMapping[temp];
