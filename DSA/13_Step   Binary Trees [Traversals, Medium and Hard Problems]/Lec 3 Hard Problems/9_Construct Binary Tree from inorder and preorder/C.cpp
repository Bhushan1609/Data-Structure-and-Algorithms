...
Node* solve(vector<int>&preorder,int pS,int pE,vector<int>&inorder,int iS,int iE,unordered_map<int,int>&mapp){
    if(iS > iE or pS > pE) return NULL;
    Node* root = new TreeNode(preorder[pS]);
    int index = mapp[preorder[pS]];
    int leftEleRemain = index - iS;
    root -> left = solve(preorder,pS+1,pS+leftEleRemain,inorder,iS,index-1,mapp);
    root -> right = solve(preorder,pS+1+leftEleRemain , pE , inorder ,index + 1 , iE,mapp);
    return root;
}
Node* buildTree(vector<int>& preorder, vector<int>& inorder) {
    ...
    Node*ans= solve(preorder,0,preorder.size()-1,inorder,0,inorder.size()-1,mapp);
    return ans;
}
void itachi_1609(){
    Node* root=buildTree({3,9,20,15,7}, {9,,3,15,20,7});    
    levelOrder(root);
    return;
}
int main(){
