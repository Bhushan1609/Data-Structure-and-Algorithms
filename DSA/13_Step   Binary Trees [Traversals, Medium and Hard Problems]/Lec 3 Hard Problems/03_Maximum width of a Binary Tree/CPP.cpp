...
int widthOfBinaryTree(Node* root) {
    ...
    q.push({root,0});
    while(!q.empty()){
        int n(len(q)),minAtLevel(1e9),maxAtLevel(-1e9);
        FOR(i,n){
            auto temp=q.front();q.pop();
            minAtLevel=min(minAtLevel,temp.se);
            maxAtLevel=max(maxAtLevel,temp.se);
            ll f=temp.se-minAtLevel;
            if(temp.fi->left)q.push({temp.fi->left,(int)2*f+1});
            if(temp.fi->right)q.push({temp.fi->right,(int)2*f+2});
        }
        ans=max(maxAtLevel-minAtLevel+1,ans);
    }
    return ans;
}
