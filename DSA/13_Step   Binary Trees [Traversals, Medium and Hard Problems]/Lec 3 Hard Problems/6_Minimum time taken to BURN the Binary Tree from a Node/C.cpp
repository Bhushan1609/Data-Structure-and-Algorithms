...
void makeParentMapping(Node* root, unordered_map<Node*,Node*>&parentMapping,Node* &address,int &target){
    if(!root)return ;
    if(root->data==target)address=root;
    if(root->left)parentMapping[root->left]=root;
    if(root->right)parentMapping[root->right]=root;
    makeParentMapping(root->left,parentMapping,address,target);
    makeParentMapping(root->right,parentMapping,address,target);
}
int minTime(Node* root, int target) {
    ...
    Node* address=NULL;
    makeParentMapping(root,parentMapping,address,target);
    q<Node*>q;
    q.push(address);
    int k=-1;
    while(!q.empty()){
        ...
        FOR(n){
            ...
            q.pop();
            visited[temp]=true;
            Node* parent=parentMapping[temp];
            if(parent && !visited[parent])q.push(parent);
