...
void widthOfBinaryTree(Node* root) {
    ...
    while(!q.empty()){
        ...
        FOR(n){
            ...
            if(_==0)first=currentNumber;
            if(_==(n-1))last=currentNumber;
            if(temp.fi->left)q.push({temp.fi->left,(currentNumber << 1)+1});
            if(temp.fi->right)q.push({temp.fi->right,(currentNumber << 1)+2});
        }
