...
void r(Node* root,unordered_map<Node*,Node*>&parentMapping){
    if(!root)return;
    if(root->left)parentMapping[root->left]=root;
    if(root->right)parentMapping[root->right]=root;
    r(root->left,parentMapping);
    r(root->right,parentMapping);
}
