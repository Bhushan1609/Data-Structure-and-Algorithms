...
int recursiveBS(int start,int end,vd<int>&arr,int target){
...
void binarySearch(vd<int>&arr,int target){
    int ans=-1;
    int start=0,end=len(arr)-1;
    while(start<=end){
        int mid=start+((end-start)>>1);
        if(arr[mid]==target){
            ans=mid;
...
void solve(){
    vd<int>arr={3,4,6,7,9,12,16,17};
    binarySearch(arr,6);
    print(recursiveBS(0,len(arr)-1,arr,6));
}
