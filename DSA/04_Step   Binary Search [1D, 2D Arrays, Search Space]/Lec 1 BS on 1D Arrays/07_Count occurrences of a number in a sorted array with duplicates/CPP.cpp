...
void callBrute(vd<int>&arr,int target){
    ...
    return print(last-first+1);
}

void callOptimal(vd<int>&arr,int target){
    int first=lower_bound(all(arr),target) - begin(arr);
    int last=(upper_bound(all(arr),target) - begin(arr)) - 1; // check Why -1
    if(first==len(arr) or arr[first] != target) return print(-1);
    return print(last-first+1);
}

void solve(){
    vd<int>arr={2,4,6,8,8,8,11,13};
    callBrute(arr,8);
    callOptimal(arr,8);
}
