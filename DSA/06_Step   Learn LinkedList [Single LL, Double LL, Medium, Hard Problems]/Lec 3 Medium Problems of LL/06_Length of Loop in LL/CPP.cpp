...
int callBrute(Node* head){
    unordered_map<Node*,int>mapp;
    Node* temp=head;
    int cnt=1;
    while(temp){
       if(mapp.find(temp) != mapp.end())return cnt-mapp[temp];
        mapp[temp]=cnt;
        cnt+=1;
        temp=temp->next;
    }
...
int giveLength(Node* slow,Node* fast){
    int cnt=1;
    slow=slow->next;
    while(slow != fast){
        cnt+=1;
...
int callOptimal(Node* head){
    Node* slow=head,*fast=head;
    while(fast && fast->next){
        fast=fast->next->next;
        slow=slow->next;
        if(fast==slow)return giveLength(slow,fast);
    }
    return 0;
}
