...
Node* callOptimal(Node* head){
    ...
    Node* memoZeroNode=zeroNode;

    Node* oneNode=new Node(-1);
    Node* memoOneNode=oneNode;

    Node* twoNode=new Node(-1);
    Node* memoTwoNode=twoNode;
    ...
    if(memoOneNode->next){
        zeroNode->next=memoOneNode->next;
    }else{
        zeroNode->next=memoTwoNode->next;
    } 
    oneNode->next=memoTwoNode->next;
    twoNode->next=NULL;
    return memoZeroNode->next;
}

void solve(){
    vd<int>vec={1,2,0,1,0,0,1,2};
