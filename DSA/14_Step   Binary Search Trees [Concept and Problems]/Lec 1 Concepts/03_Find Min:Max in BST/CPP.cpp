...
pi<int,int>minMax(Node* root){
    Node* r1=root;
    Node* r2=root;
    pi<int,int>answer={1e9,-1e9};
    while(r1){
        answer.fi=min(answer.fi,root->data);
        r1=r1->left;
    }
    while(r2){
        answer.se=min(answer.se,root->data);
        r2=r2->right;
    }
    return answer;
}
...
void solve(){
    ...
    // levelOrder(root);
    // pi<int,int>ans=minMax(root)
    // return print(ans.fi,ans.se);
}
