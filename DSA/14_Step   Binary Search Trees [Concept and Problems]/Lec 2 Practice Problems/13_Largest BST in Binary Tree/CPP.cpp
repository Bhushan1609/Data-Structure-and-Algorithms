...
pi<int,pi<int,int>> postOrder(Node* root){
    if(!root)return {0,{1e9,-1e9}};
    pi<int,pi<int,int>> leftAns=postOrder(root->left);
    pi<int,pi<int,int>> rightAns=postOrder(root->right);
    if(leftAns.se.se < root->data && root->data < rightAns.se.fi){
        return {leftAns.fi+rightAns.fi+1,{min(leftAns.se.fi,root->data) , max(root->data,rightAns.se.se)}};
    }
    return {max(leftAns.fi,rightAns.fi),{-1e9,1e9}};
}

int largestBst(Node *root){
    return (int)postOrder(root).fi;
}
