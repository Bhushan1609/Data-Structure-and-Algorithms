...
bool findTarget(TreeNode* root, int k) {
    BSTIterator lowSort(root,true);
    BSTIterator highSort(root,false);
    int low=lowSort.nextORbefore();
    int high=highSort.nextORbefore();
    while(low<high){
        if((low+high)==k) return 1;
        if((low+high) < k){
            low=lowSort.nextORbefore();
        }else{
            high=highSort.nextORbefore();
        }
    }
    return 0;
}
