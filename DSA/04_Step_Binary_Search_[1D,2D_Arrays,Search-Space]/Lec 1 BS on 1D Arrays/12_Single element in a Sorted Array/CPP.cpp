...
void callBrute(vd<int>&arr){
    if(len(arr)==1) return print(arr[0]);
    int ans=-1;
    FOR(i,len(arr)){
        if(i==0){
            if(arr[i] != arr[i+1]){
...
void callOptimal(vd<int>&arr){
    if(len(arr)==1) return print(arr[0]);
    if(arr[0] != arr[1]) return print(arr[0]);
    if(arr[len(arr)-2] != arr[len(arr)-1]) return print(arr[len(arr)-1]);
    int start=1,end=len(arr)-2;
    while(start<=end){
        int mid=start+((end-start)>>1);
        if(arr[mid] != arr[mid+1] && arr[mid] != arr[mid-1]) return print(arr[mid]);
...
void solve(){
    vd<int>arr={1,1,2,2,3,3,4,5,5,6,6};
    callBrute(arr);
    callOptimal(arr);
}
