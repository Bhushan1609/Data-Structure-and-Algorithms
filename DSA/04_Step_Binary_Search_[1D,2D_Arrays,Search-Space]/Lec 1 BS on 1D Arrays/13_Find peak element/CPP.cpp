...
void callBrute(vd<int>&arr){
    FOR(i,len(arr)){
        if((i==0 or arr[i-1]<arr[i]) && (i==len(arr)-1 or arr[i]>arr[i+1])){
            return print(i);
        }
    }
    return print(-1);
...
void callOptimal(vd<int>&arr){
    if(len(arr)==1) return print(0);
    if(arr[0]>arr[1]) return print(0);
    if(arr[len(arr)-1]>arr[len(arr)-2]) return print(len(arr)-1);
...
void solve(){
    vd<int>arr={1,2,3,4,5,6,7,8,5,1};
    callBrute(arr);
    callOptimal(arr);
}
